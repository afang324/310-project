    ...
    constructor(collection){
        collection.find({},{},function(err,docs){
            for (var i=0;i<docs.length;i++){
                ...
                (function (i){
                    var address:string = docs[i].address
                    if(docs[i]!=undefined){
                        var nolatlng:boolean = docs[i].lat==null || docs[i].lng== null;
                        if(nolatlng){
                            geocoder.geocode(address, function(err, res) {
                                ...
                                var type:string = docs[i].type;
                                var space:number = docs[i].space;
                                var comment:string = docs[i].comment;
                                //insert converted or default value into db      
                                collection.update({"address":address,"lat":null,"lng":null},
                                    {$set:{"lat":lat,"lng":lng}}
                                );
                            });
                        }
